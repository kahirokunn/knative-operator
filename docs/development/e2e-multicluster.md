...
The hub namespace identifies only the management CR. Serving is installed in
`knative-serving` on the spoke. The same hub namespace can hold the Eventing
management CR, whose resources are installed in `knative-eventing`:
