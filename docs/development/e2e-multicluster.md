...
The `KnativeServing` exists only as `fleet-workloads/serving-spoke` on the hub.
The operator installs its workloads into `knative-serving` on the spoke.

Use the same hub namespace for an Eventing management CR while keeping the
canonical Eventing namespace on the spoke:
