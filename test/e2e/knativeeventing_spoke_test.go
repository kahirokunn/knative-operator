...
func TestMulticlusterKnativeEventingSpokeDeployment(t *testing.T) {
	...
	if err := createKnativeEventingWithPlacement(ctx, hub, names); err != nil {
	...
	t.Run("hub-cr-ready", func(t *testing.T) {
		...
		waitForTargetClusterCondition(t.Context(), t, eventingTargetCluster(hub, names), corev1.ConditionTrue, "")
	})
	...
	t.Run("spoke-runtime-resources-owned-by-anchor", func(t *testing.T) {
		waitForSpokeRuntimeResourcesOwned(t.Context(), t, spoke, newSpokeKnativeEventing(names))
	})
	...
	t.Run("delete-and-cleanup-spoke", func(t *testing.T) {
		...
		if err := waitForSpokeDeploymentsGone(ctx, t, spoke, spokeEventingInstallNamespace); err != nil {
			t.Fatalf("Spoke deployments still present after deletion in namespace %q: %v",
				spokeEventingInstallNamespace, err)
		}
		waitForSpokeRuntimeResourcesGone(ctx, t, spoke, spokeEventingInstallNamespace)
		assertAnchorConfigMapGone(ctx, t, spoke, newSpokeKnativeEventing(names))
	})
}

func newSpokeKnativeEventing(names test.ResourceNames) *v1beta1.KnativeEventing {
	return &v1beta1.KnativeEventing{
...
func createKnativeEventingWithPlacement(ctx context.Context, clients *test.Clients, names test.ResourceNames) error {
	_, err := clients.KnativeEventing().Create(ctx, newSpokeKnativeEventing(names), metav1.CreateOptions{})
...
func waitForSpokeEventingDeploymentsReady(ctx context.Context, t *testing.T, hub *test.Clients, spoke *test.Clients, names test.ResourceNames) {
	t.Helper()
	waitForTargetClusterCondition(ctx, t, eventingTargetCluster(hub, names), corev1.ConditionTrue, "")
	waitForSpokeDeploymentsAvailable(ctx, t, spoke, spokeEventingInstallNamespace)
}
