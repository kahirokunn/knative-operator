...
func TestApplyResourcePatchesAppliesAllTypesInOrder(t *testing.T) {
	manifest := manifestForPatchTest(t, deploymentForPatchTest("activator", "knative-serving", 1))
...
func TestApplyResourcePatchesUsesNamespaceToDisambiguateTarget(t *testing.T) {
	manifest := manifestForPatchTest(t,
		deploymentForPatchTest("activator", "one", 1),
		deploymentForPatchTest("activator", "two", 1),
	)
...
func TestApplyResourcePatchesRemovesReplicas(t *testing.T) {
	manifest := manifestForPatchTest(t, deploymentForPatchTest("activator", "knative-serving", 3))
...
`)
	...
	if _, found, err := unstructured.NestedFieldNoCopy(manifest.Resources()[0].Object, "spec", "replicas"); err != nil || found {
...
func TestApplyResourcePatchesDeletesResource(t *testing.T) {
	...
	deployment := deploymentForPatchTest("activator", "knative-serving", 1)
...
func TestApplyResourcePatchesValidatesTargetAndIdentity(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			resources: []unstructured.Unstructured{deploymentForPatchTest("controller", "knative-serving", 1)},
			patch:     resourcePatchForTest(base.MergePatchType, "apps/v1", "Deployment", "activator", `{}`),
			wantError: "did not match any generated resource",
		},
		{
			name: "target is ambiguous",
			resources: []unstructured.Unstructured{
				deploymentForPatchTest("activator", "one", 1),
				deploymentForPatchTest("activator", "two", 1),
			},
		...
		},
		{
			...
			resources: []unstructured.Unstructured{deploymentForPatchTest("activator", "knative-serving", 1)},
			patch: resourcePatchForTest(base.MergePatchType, "apps/v1", "Deployment", "activator", `
metadata:
  name: changed
`),
...
`),
	...
	manifest := manifestForPatchTest(t, deploymentForPatchTest("controller", "upstream-namespace", 1))
...
func TestManifestivalPreservesExternallyManagedReplicas(t *testing.T) {
	...
	identity := deploymentForPatchTestWithoutReplicas("activator", "knative-serving")
	live, err := client.Get(&identity)
	if err != nil {
		t.Fatalf("Get() = %v", err)
	}
	if err := unstructured.SetNestedField(live.Object, int64(7), "spec", "replicas"); err != nil {
		t.Fatalf("SetNestedField() = %v", err)
	}
	if err := client.Update(live); err != nil {
	...
	if replicas, _, err := unstructured.NestedInt64(live.Object, "spec", "replicas"); err != nil || replicas != 7 {
...
func deploymentForPatchTest(name, namespace string, replicas int64) unstructured.Unstructured {
			...
			"template": map[string]interface{}{
				...
				"spec": map[string]interface{}{
					"containers": []interface{}{
						map[string]interface{}{"name": name, "image": "example.com/image"},
					},
				},
			},
		},
