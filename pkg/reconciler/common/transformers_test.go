...
func TestCommonTransformers(t *testing.T) {
	...
	// InjectOwner is now an extra transformer (no longer in transformers()).
	...
	// Verify OwnerReference is set (from the first Transform call with InjectOwner).
	// Re-apply InjectOwner to verify it works with the platform extension too.
	if err := Transform(context.Background(), &manifest, component, InjectOwner(component, nil)); err != nil {
		t.Fatalf("Failed to transform manifest: %v", err)
	}
	resource = &manifest.Resources()[0]
...
func TestInjectOwner_UsesAnchorWhenSet(t *testing.T) {
	// When anchorOwner is set, OwnerReferences should point to the anchor.
	component := &v1beta1.KnativeEventing{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: "test-ns",
			Name:      "test-name",
	...
	if len(resource.GetOwnerReferences()) == 0 {
		t.Fatal("Expected OwnerReferences to be set when anchorOwner is provided")
	}
...
func TestInjectOwner_UsesCRWhenAnchorNil(t *testing.T) {
	// Without anchorOwner, InjectOwner must set OwnerReferences to the CR itself.
	component := &v1beta1.KnativeEventing{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: "test-ns",
			Name:      "test-name",
	...
	resource := &m.Resources()[0]
	if len(resource.GetOwnerReferences()) == 0 {
		t.Fatal("Expected OwnerReferences to be set when anchorOwner is nil")
	}
...
func TestInjectOwner_SkipsClusterScoped(t *testing.T) {
	// Cluster-scoped resources should not have OwnerReferences set,
	// regardless of whether anchorOwner is provided.
	component := &v1beta1.KnativeEventing{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: "test-ns",
			Name:      "test-name",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resource := &m.Resources()[0]
			if len(resource.GetOwnerReferences()) != 0 {
				t.Fatalf("Expected no OwnerReferences for cluster-scoped resource, got %d",
